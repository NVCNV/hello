...
MRSQL="
(select cellid,rruid,gridid,(eupordown)dir_state,xdrid,
...
sum(case when MRNAME='MR.LteScRSRP' AND KPI1 is not NULL AND KPI2 is not NULL and (kpi1-141)>-110 and abs(kpi1-kpi2)=6 then 1 else 0 end)overlapcoverratex,
...
abs(KPI1 - KPI2) < 6 then 1 else 0 end)model3diststroy
          from $addRRUID
                 group by
                  CELLID,rruid,gridid,eupordown,xdrid)t
"
hive<<EOF
...
alter table mro_kpi_mid_cell_grid_hour set serdeproperties('serialization.null.format' = '');
insert into mro_kpi_mid_cell_grid_hour partition(dt="$ANALY_DATE",h="$ANALY_HOUR")
(ttime,cellid,rruid,gridid,dir_state,kpi001,kpi002,kpi003,kpi004,kpi005,kpi006,kpi007,kpi010,kpi011,kpi012,
kpi013,kpi014,kpi015,kpi016,kpi017,kpi018,kpi019
)
select "$CAL_DATE",cellid,rruid,gridid,dir_state,sum(avgrsrpx),sum(commy),sum(avgrsrqx),sum(ltecoverratex),sum(weakcoverratex),
...
from $MRSQL group by cellid,rruid,gridid,dir_state;
EOF
