...
uusql="
select imsi,msisdn,cellid,railline,(eupordown)dir_state,0 as voltemcsucc,0 as voltemcatt,0 as voltevdsucc,0 as voltevdatt,0 as voltetime,
...
0 AS srvccsuccS1
          ...
          WHERE dt="$ANALY_DATE" and h="$ANALY_HOUR"
                 group by 
                  imsi,msisdn,CELLID,railline,eupordown
"
x2sql="
select imsi,msisdn,CELLID,railline,(eupordown)dir_state,0 as voltemcsucc,0 as voltemcatt,0 as voltevdsucc,0 as voltevdatt,0 as voltetime,
0 as voltemctime,0 as voltemctimey,0 as voltevdtime,0 as voltevdtimey,
...
"
svsql="
select imsi,msisdn,(SOURCEECI)cellid,railline,(eupordown)dir_state,0 as voltemcsucc,0 as voltemcatt,0 as voltevdsucc,0 as voltevdatt,0 as voltetime,0 as voltemctime,0 as voltemctimey,0 as voltevdtime,0 as voltevdtimey,
0 as voltemchandover,0 as volteanswer,0 as voltevdhandover,0 as voltevdanswer,
0 as srvccsucc,
0 as srvccatt,
sum(case when ProcedureType=1 and RESULT=0 then SVDELAY/100 end)srvcctime,
0 as lteswsucc,0 as lteswatt,0 as srqatt ,0 as srqsucc ,0 as tauatt  ,0 as tausucc,0 as rrcrebuild,0 as rrcsucc,0 as rrcreq,0 as imsiregatt,0 as imsiregsucc,
...
0 AS srvccsuccS1
          ...
          WHERE dt="$ANALY_DATE" and h="$ANALY_HOUR"
                 group by 
                  imsi,msisdn,SOURCEECI,railline,eupordown
"

voltesipsql="
select imsi,msisdn,(sourceeci)cellid,railline,(eupordown)dir_state,
0 as voltemcsucc,
0 as voltemcatt,
0 as voltevdsucc,
0 as voltevdatt,
...
sum(case when ProcedureType=5 and interface=14 and alertingtime is not null then ALERTINGTIME/100 else 0 end)voltetime,
sum(case when ProcedureType=5 and interface=14 and ServiceType=0 and callduration<>4294967295 then callduration/100 else 0 end)voltemctime,
...
voltesip1sql="
select imsi,msisdn,(desteci)cellid,railline,(eupordown)dir_state,
sum(case when ProcedureType=5 and interface=14 and ServiceType=0 and alertingtime is not null then 1 else 0 end)voltemcsucc,
sum(case when ProcedureType=5 and interface=14 and ServiceType=0 then 1 else 0 end)voltemcatt,
sum(case when ProcedureType=5 and interface=14 and ServiceType=1 and alertingtime is not null then 1 else 0 end)voltevdsucc,
sum(case when ProcedureType=5 and interface=14 and ServiceType=1 then 1 else 0 end)voltevdatt,
...
0 AS srvccsuccS1
       ...
       where callside=1 and dt="$ANALY_DATE" and h="$ANALY_HOUR"
                 group by 
                  imsi,msisdn,desteci,railline,eupordown
"

#excel sheet����VOLTE_Rx�ӿ�С������
#volterxsql="(select imsi,msisdn,ECGI,railline,(eupordown)dir_state,0 as voltemcsucc, 0 as voltemcatt, 0 as voltevdsucc, 0 as voltevdatt, 0 as voltetime, 0 as voltemctime, 0 as voltevdtime,
...
#           WHERE dt="$ANALY_DATE" and h="$ANALY_HOUR"
#              group by 
#                  imsi,msisdn,ECGI,railline,dir_state)"


s1mmesql="select imsi,msisdn,CELLID,railline,(eupordown)dir_state,0 as voltemcsucc, 0 as voltemcatt, 0 as voltevdsucc, 0 as voltevdatt,
...
S1MMEHANDOVER="
select imsi,msisdn,CELLID,railline,(eupordown)dir_state,
0 as voltemcsucc, 
0 as voltemcatt, 
0 as voltevdsucc, 
0 as voltevdatt, 
...
group by imsi,msisdn,CELLID,railline,eupordown
"
...
(ttime,imsi,msisdn,cellid,railline,dir_state,kpi031,kpi032,kpi033,kpi034,kpi035,kpi037,kpi039,kpi041,
...
select "$CAL_DATE",imsi,msisdn,CELLID,railline,dir_state,sum(voltemcsucc),sum(voltemcatt),sum(voltevdsucc),sum(voltevdatt),sum(voltetime),
...
group by imsi,msisdn,cellid,railline,dir_state;
EOF
