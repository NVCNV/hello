...
class badqualitycellid(ANALY_DATE: String, ANALY_HOUR: String, SDB: String, DDB: String, warhouseDir: String){
  def analyse(implicit sparkSession: SparkSession): Unit ={
    ...
    sql(s"alter table $DDB.exception_analysis add if not exists partition(dt=$ANALY_DATE,h=$ANALY_HOUR)")
    sql(s"use $DDB")
    sql(
      s"""
         |select
...
//       """.stripMargin)
    ...
    val lowspeed_table= sql(
      s"""
         |select cellid,(case when (sum(case when gtuser_flag=1 then 1 else 0 end)/count(1))<0.5 then 3 else 0 end)etype from gt_pulse_detail where dt=$ANALY_DATE and h=$ANALY_HOUR group by cellid
       """.stripMargin)
