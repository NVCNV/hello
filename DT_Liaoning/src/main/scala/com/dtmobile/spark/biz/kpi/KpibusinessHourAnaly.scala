  ...
  def tacHourAnalyse(implicit sparkSession: SparkSession): Unit = {
    ...
    sql(
      s"""
         ...
         |sum(case when Interface = 11 and APPTYPECODE = 103 and APPTYPE = 15 and DLDATA is not null and ULDATA is not null then DLDATA+ULDATA end )browsedownloadbusiness,
         |sum(case when Interface = 11 and (APPTYPECODE = 103 or APPTYPECODE = 107)and APPTYPE = 5 and DLDATA is not null and ULDATA is not null then DLDATA+ULDATA end )videoservicebusiness,
         |sum(case when Interface = 11 and APPTYPECODE = 108 and APPTYPE = 1 and DLDATA is not null and ULDATA is not null then DLDATA+ULDATA end )instantmessagebusiness,
         |sum(case when Interface = 11 and APPTYPECODE = 103 and APPTYPE = 7 and DLDATA is not null and ULDATA is not null then DLDATA+ULDATA end )appbusiness,
         |0 as dnsQuerySucc,
         |0 as dnsQueryAtt,
