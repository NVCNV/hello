  ...
  def tacHourAnalyse(implicit sparkSession: SparkSession): Unit = {
    ...
    sql(
      s"""
         ...
         |sum(case when Interface = 11 and APPTYPECODE = 108 and APPTYPE = 1 and DLDATA is not null and ULDATA is not null then DLDATA+ULDATA else 0 end )instantmessagebusiness,
