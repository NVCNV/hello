...
object threed_mrlocate_floor {
  def main(args: Array[String]) {
    ...
    hiveContext.sql("create table if not exists mrlocate_floor (objectid bigint, mmeues1apid string," +
    ...
    while (height <=  AppSettings.maxHeight) {
      ...
      hiveContext.sql("create table if not exists finger_total" + height + " (objectid int, gridid bigint, longitude double, latitude double, rsrp double, n_objectid int, n_rsrp int) ROW FORMAT DELIMITED FIELDS TERMINATED BY ','")
      ...
      hiveContext.sql(" insert into table mrlocate_floor PARTITION (day = '"+day+"', hour = '"+hour+"') " +
