...
class KpiDayAnaly(ANALY_DATE: String,SDB: String, DDB: String, warhouseDir: String) {
  ...
  def mrImsiDayAnalyse(implicit sparkSession: SparkSession): Unit = {
    import sparkSession.sql
    sql(s"use $DDB")
    sql(s"alter table mro_kpi_mid_imsi_day add if not exists partition(dt=$ANALY_DATE)")
    sql(
      s"""
         |SELECT
         |	imsi,
    ...
    sql(
      s"""
         ...
         |	imsi,
         |	msisdn
       """.stripMargin).write.mode(SaveMode.Overwrite).csv(s"$warhouseDir/mro_kpi_mid_imsi_day/dt=$ANALY_DATE")
  }
